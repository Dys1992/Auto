package model.flightresponsemodel;

/**
 * Auto-generated: 2018-01-23 19:8:34
 *
 * @author bejson.com (i@bejson.com)
 * @website http://www.bejson.com/java2pojo/
 */
public class Cabins {

    private String realRoomCode;
    private String roomDes;
    private String ticketsNum;
    private String discountRate;
    private String clientTicketPrice;
    private String clientManPrice;
    private String roomRuleId;
    private String fProductCode;
    private String fProductID;
    private int fzRuleID;
    private String fProductName;
    private String FOEShapeType;
    private String rT;
    private String rpt;
    private int spt;
    private String brc;
    private int sbatp;
    private int smtp;
    private int sctp;
    private int sbtp;
    private String tmp;
    private String tcp;
    private String tbp;
    private String ccp;
    private String cbp;
    private int aid;
    private String trb;
    private long mid;
    private int nkstp;
    private String fkd;
    private String fkp;
    private int dmp;
    private int dcmp;
    private String spc;
    private int pt;
    private String obp;
    private String ocbp;
    private int opi;
    private String zp;
    private String czp;
    private String zit;
    private String zv;
    private int proid;
    private int intax;
    private String cnptp;
    private String fpf;
    private int srmtp;
    private int isbt;
    private String fzjjp;
    private String sm;
    private int fat;
    private int cuc;
    private int redp;
    private int fmo;
    private int ibt;
    private String fwp;
    private String faid;
    private String fpoid;
    private int fpt;
    private int tot;
    private int pae;
    private int ibs;
    private String sst;
    private String set;
    private int hl;
    private int curp;
    private int mt;
    private int ge;
    private String fn;
    private String pd;
    private String cr;
    private String cc;
    private String ce;
    private String prm;
    private int smsTemp;
    private int pm;
    private String fa;
    private String fabl;
    private String bigcc;
    private String offnum;
    private int isagbook;
    private boolean isGrab;
    private String gwyh;
    private int rbt;
    private String thrbm;
    private String chrbm;
    private String backthrbm;
    private String backchrbm;
    private String extinfo;
    private int BaseRoomType;
    private int prstype;
    private int spkey;
    private int smcprice;
    private int isMCE;
    private int issvp;
    private String realLevel;
    private String encryptstring;
    private String gwpc;
    private int reclabel;
    private String si;
    private int ifg;
    private String sdl;
    private int am;
    private String ml;
    private String mlr;
    private int mlid;
    private String mln;
    private String rctp;
    private int flp;
    private String sicons;
    private int iscom;
    private int gwsf;
    private int mtype;
    private int pricetype;
    public void setRealRoomCode(String realRoomCode) {
         this.realRoomCode = realRoomCode;
     }
     public String getRealRoomCode() {
         return realRoomCode;
     }

    public void setRoomDes(String roomDes) {
         this.roomDes = roomDes;
     }
     public String getRoomDes() {
         return roomDes;
     }

    public void setTicketsNum(String ticketsNum) {
         this.ticketsNum = ticketsNum;
     }
     public String getTicketsNum() {
         return ticketsNum;
     }

    public void setDiscountRate(String discountRate) {
         this.discountRate = discountRate;
     }
     public String getDiscountRate() {
         return discountRate;
     }

    public void setClientTicketPrice(String clientTicketPrice) {
         this.clientTicketPrice = clientTicketPrice;
     }
     public String getClientTicketPrice() {
         return clientTicketPrice;
     }

    public void setClientManPrice(String clientManPrice) {
         this.clientManPrice = clientManPrice;
     }
     public String getClientManPrice() {
         return clientManPrice;
     }

    public void setRoomRuleId(String roomRuleId) {
         this.roomRuleId = roomRuleId;
     }
     public String getRoomRuleId() {
         return roomRuleId;
     }

    public void setFProductCode(String fProductCode) {
         this.fProductCode = fProductCode;
     }
     public String getFProductCode() {
         return fProductCode;
     }

    public void setFProductID(String fProductID) {
         this.fProductID = fProductID;
     }
     public String getFProductID() {
         return fProductID;
     }

    public void setFzRuleID(int fzRuleID) {
         this.fzRuleID = fzRuleID;
     }
     public int getFzRuleID() {
         return fzRuleID;
     }

    public void setFProductName(String fProductName) {
         this.fProductName = fProductName;
     }
     public String getFProductName() {
         return fProductName;
     }

    public void setFOEShapeType(String FOEShapeType) {
         this.FOEShapeType = FOEShapeType;
     }
     public String getFOEShapeType() {
         return FOEShapeType;
     }

    public void setRT(String rT) {
         this.rT = rT;
     }
     public String getRT() {
         return rT;
     }

    public void setRpt(String rpt) {
         this.rpt = rpt;
     }
     public String getRpt() {
         return rpt;
     }

    public void setSpt(int spt) {
         this.spt = spt;
     }
     public int getSpt() {
         return spt;
     }

    public void setBrc(String brc) {
         this.brc = brc;
     }
     public String getBrc() {
         return brc;
     }

    public void setSbatp(int sbatp) {
         this.sbatp = sbatp;
     }
     public int getSbatp() {
         return sbatp;
     }

    public void setSmtp(int smtp) {
         this.smtp = smtp;
     }
     public int getSmtp() {
         return smtp;
     }

    public void setSctp(int sctp) {
         this.sctp = sctp;
     }
     public int getSctp() {
         return sctp;
     }

    public void setSbtp(int sbtp) {
         this.sbtp = sbtp;
     }
     public int getSbtp() {
         return sbtp;
     }

    public void setTmp(String tmp) {
         this.tmp = tmp;
     }
     public String getTmp() {
         return tmp;
     }

    public void setTcp(String tcp) {
         this.tcp = tcp;
     }
     public String getTcp() {
         return tcp;
     }

    public void setTbp(String tbp) {
         this.tbp = tbp;
     }
     public String getTbp() {
         return tbp;
     }

    public void setCcp(String ccp) {
         this.ccp = ccp;
     }
     public String getCcp() {
         return ccp;
     }

    public void setCbp(String cbp) {
         this.cbp = cbp;
     }
     public String getCbp() {
         return cbp;
     }

    public void setAid(int aid) {
         this.aid = aid;
     }
     public int getAid() {
         return aid;
     }

    public void setTrb(String trb) {
         this.trb = trb;
     }
     public String getTrb() {
         return trb;
     }

    public void setMid(long mid) {
         this.mid = mid;
     }
     public long getMid() {
         return mid;
     }

    public void setNkstp(int nkstp) {
         this.nkstp = nkstp;
     }
     public int getNkstp() {
         return nkstp;
     }

    public void setFkd(String fkd) {
         this.fkd = fkd;
     }
     public String getFkd() {
         return fkd;
     }

    public void setFkp(String fkp) {
         this.fkp = fkp;
     }
     public String getFkp() {
         return fkp;
     }

    public void setDmp(int dmp) {
         this.dmp = dmp;
     }
     public int getDmp() {
         return dmp;
     }

    public void setDcmp(int dcmp) {
         this.dcmp = dcmp;
     }
     public int getDcmp() {
         return dcmp;
     }

    public void setSpc(String spc) {
         this.spc = spc;
     }
     public String getSpc() {
         return spc;
     }

    public void setPt(int pt) {
         this.pt = pt;
     }
     public int getPt() {
         return pt;
     }

    public void setObp(String obp) {
         this.obp = obp;
     }
     public String getObp() {
         return obp;
     }

    public void setOcbp(String ocbp) {
         this.ocbp = ocbp;
     }
     public String getOcbp() {
         return ocbp;
     }

    public void setOpi(int opi) {
         this.opi = opi;
     }
     public int getOpi() {
         return opi;
     }

    public void setZp(String zp) {
         this.zp = zp;
     }
     public String getZp() {
         return zp;
     }

    public void setCzp(String czp) {
         this.czp = czp;
     }
     public String getCzp() {
         return czp;
     }

    public void setZit(String zit) {
         this.zit = zit;
     }
     public String getZit() {
         return zit;
     }

    public void setZv(String zv) {
         this.zv = zv;
     }
     public String getZv() {
         return zv;
     }

    public void setProid(int proid) {
         this.proid = proid;
     }
     public int getProid() {
         return proid;
     }

    public void setIntax(int intax) {
         this.intax = intax;
     }
     public int getIntax() {
         return intax;
     }

    public void setCnptp(String cnptp) {
         this.cnptp = cnptp;
     }
     public String getCnptp() {
         return cnptp;
     }

    public void setFpf(String fpf) {
         this.fpf = fpf;
     }
     public String getFpf() {
         return fpf;
     }

    public void setSrmtp(int srmtp) {
         this.srmtp = srmtp;
     }
     public int getSrmtp() {
         return srmtp;
     }

    public void setIsbt(int isbt) {
         this.isbt = isbt;
     }
     public int getIsbt() {
         return isbt;
     }

    public void setFzjjp(String fzjjp) {
         this.fzjjp = fzjjp;
     }
     public String getFzjjp() {
         return fzjjp;
     }

    public void setSm(String sm) {
         this.sm = sm;
     }
     public String getSm() {
         return sm;
     }

    public void setFat(int fat) {
         this.fat = fat;
     }
     public int getFat() {
         return fat;
     }

    public void setCuc(int cuc) {
         this.cuc = cuc;
     }
     public int getCuc() {
         return cuc;
     }

    public void setRedp(int redp) {
         this.redp = redp;
     }
     public int getRedp() {
         return redp;
     }

    public void setFmo(int fmo) {
         this.fmo = fmo;
     }
     public int getFmo() {
         return fmo;
     }

    public void setIbt(int ibt) {
         this.ibt = ibt;
     }
     public int getIbt() {
         return ibt;
     }

    public void setFwp(String fwp) {
         this.fwp = fwp;
     }
     public String getFwp() {
         return fwp;
     }

    public void setFaid(String faid) {
         this.faid = faid;
     }
     public String getFaid() {
         return faid;
     }

    public void setFpoid(String fpoid) {
         this.fpoid = fpoid;
     }
     public String getFpoid() {
         return fpoid;
     }

    public void setFpt(int fpt) {
         this.fpt = fpt;
     }
     public int getFpt() {
         return fpt;
     }

    public void setTot(int tot) {
         this.tot = tot;
     }
     public int getTot() {
         return tot;
     }

    public void setPae(int pae) {
         this.pae = pae;
     }
     public int getPae() {
         return pae;
     }

    public void setIbs(int ibs) {
         this.ibs = ibs;
     }
     public int getIbs() {
         return ibs;
     }

    public void setSst(String sst) {
         this.sst = sst;
     }
     public String getSst() {
         return sst;
     }

    public void setSet(String set) {
         this.set = set;
     }
     public String getSet() {
         return set;
     }

    public void setHl(int hl) {
         this.hl = hl;
     }
     public int getHl() {
         return hl;
     }

    public void setCurp(int curp) {
         this.curp = curp;
     }
     public int getCurp() {
         return curp;
     }

    public void setMt(int mt) {
         this.mt = mt;
     }
     public int getMt() {
         return mt;
     }

    public void setGe(int ge) {
         this.ge = ge;
     }
     public int getGe() {
         return ge;
     }

    public void setFn(String fn) {
         this.fn = fn;
     }
     public String getFn() {
         return fn;
     }

    public void setPd(String pd) {
         this.pd = pd;
     }
     public String getPd() {
         return pd;
     }

    public void setCr(String cr) {
         this.cr = cr;
     }
     public String getCr() {
         return cr;
     }

    public void setCc(String cc) {
         this.cc = cc;
     }
     public String getCc() {
         return cc;
     }

    public void setCe(String ce) {
         this.ce = ce;
     }
     public String getCe() {
         return ce;
     }

    public void setPrm(String prm) {
         this.prm = prm;
     }
     public String getPrm() {
         return prm;
     }

    public void setSmsTemp(int smsTemp) {
         this.smsTemp = smsTemp;
     }
     public int getSmsTemp() {
         return smsTemp;
     }

    public void setPm(int pm) {
         this.pm = pm;
     }
     public int getPm() {
         return pm;
     }

    public void setFa(String fa) {
         this.fa = fa;
     }
     public String getFa() {
         return fa;
     }

    public void setFabl(String fabl) {
         this.fabl = fabl;
     }
     public String getFabl() {
         return fabl;
     }

    public void setBigcc(String bigcc) {
         this.bigcc = bigcc;
     }
     public String getBigcc() {
         return bigcc;
     }

    public void setOffnum(String offnum) {
         this.offnum = offnum;
     }
     public String getOffnum() {
         return offnum;
     }

    public void setIsagbook(int isagbook) {
         this.isagbook = isagbook;
     }
     public int getIsagbook() {
         return isagbook;
     }

    public void setIsGrab(boolean isGrab) {
         this.isGrab = isGrab;
     }
     public boolean getIsGrab() {
         return isGrab;
     }

    public void setGwyh(String gwyh) {
         this.gwyh = gwyh;
     }
     public String getGwyh() {
         return gwyh;
     }

    public void setRbt(int rbt) {
         this.rbt = rbt;
     }
     public int getRbt() {
         return rbt;
     }

    public void setThrbm(String thrbm) {
         this.thrbm = thrbm;
     }
     public String getThrbm() {
         return thrbm;
     }

    public void setChrbm(String chrbm) {
         this.chrbm = chrbm;
     }
     public String getChrbm() {
         return chrbm;
     }

    public void setBackthrbm(String backthrbm) {
         this.backthrbm = backthrbm;
     }
     public String getBackthrbm() {
         return backthrbm;
     }

    public void setBackchrbm(String backchrbm) {
         this.backchrbm = backchrbm;
     }
     public String getBackchrbm() {
         return backchrbm;
     }

    public void setExtinfo(String extinfo) {
         this.extinfo = extinfo;
     }
     public String getExtinfo() {
         return extinfo;
     }

    public void setBaseRoomType(int BaseRoomType) {
         this.BaseRoomType = BaseRoomType;
     }
     public int getBaseRoomType() {
         return BaseRoomType;
     }

    public void setPrstype(int prstype) {
         this.prstype = prstype;
     }
     public int getPrstype() {
         return prstype;
     }

    public void setSpkey(int spkey) {
         this.spkey = spkey;
     }
     public int getSpkey() {
         return spkey;
     }

    public void setSmcprice(int smcprice) {
         this.smcprice = smcprice;
     }
     public int getSmcprice() {
         return smcprice;
     }

    public void setIsMCE(int isMCE) {
         this.isMCE = isMCE;
     }
     public int getIsMCE() {
         return isMCE;
     }

    public void setIssvp(int issvp) {
         this.issvp = issvp;
     }
     public int getIssvp() {
         return issvp;
     }

    public void setRealLevel(String realLevel) {
         this.realLevel = realLevel;
     }
     public String getRealLevel() {
         return realLevel;
     }

    public void setEncryptstring(String encryptstring) {
         this.encryptstring = encryptstring;
     }
     public String getEncryptstring() {
         return encryptstring;
     }

    public void setGwpc(String gwpc) {
         this.gwpc = gwpc;
     }
     public String getGwpc() {
         return gwpc;
     }

    public void setReclabel(int reclabel) {
         this.reclabel = reclabel;
     }
     public int getReclabel() {
         return reclabel;
     }

    public void setSi(String si) {
         this.si = si;
     }
     public String getSi() {
         return si;
     }

    public void setIfg(int ifg) {
         this.ifg = ifg;
     }
     public int getIfg() {
         return ifg;
     }

    public void setSdl(String sdl) {
         this.sdl = sdl;
     }
     public String getSdl() {
         return sdl;
     }

    public void setAm(int am) {
         this.am = am;
     }
     public int getAm() {
         return am;
     }

    public void setMl(String ml) {
         this.ml = ml;
     }
     public String getMl() {
         return ml;
     }

    public void setMlr(String mlr) {
         this.mlr = mlr;
     }
     public String getMlr() {
         return mlr;
     }

    public void setMlid(int mlid) {
         this.mlid = mlid;
     }
     public int getMlid() {
         return mlid;
     }

    public void setMln(String mln) {
         this.mln = mln;
     }
     public String getMln() {
         return mln;
     }

    public void setRctp(String rctp) {
         this.rctp = rctp;
     }
     public String getRctp() {
         return rctp;
     }

    public void setFlp(int flp) {
         this.flp = flp;
     }
     public int getFlp() {
         return flp;
     }

    public void setSicons(String sicons) {
         this.sicons = sicons;
     }
     public String getSicons() {
         return sicons;
     }

    public void setIscom(int iscom) {
         this.iscom = iscom;
     }
     public int getIscom() {
         return iscom;
     }

    public void setGwsf(int gwsf) {
         this.gwsf = gwsf;
     }
     public int getGwsf() {
         return gwsf;
     }

    public void setMtype(int mtype) {
         this.mtype = mtype;
     }
     public int getMtype() {
         return mtype;
     }

    public void setPricetype(int pricetype) {
         this.pricetype = pricetype;
     }
     public int getPricetype() {
         return pricetype;
     }

}